...
BASE = "https://online.sefaz.am.gov.br/sintegra/"


def buscar_inscricao_estadual(cnpj):
    try:
        session = nova_sessao()
        pagina = session.get(BASE + "index.asp", timeout=15)
        if "recaptcha" in pagina.text.lower():
            return {"erro": "Portal exige captcha"}

        response = session.post(BASE + "sintegra_cnpj.asp", data={"cgc": cnpj}, timeout=15)
